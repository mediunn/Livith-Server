...
export class NotificationQueueScheduler{
    ...
    async processQueue(){
        ...
        for(const row of rows){
            try{
                if(row.eventType === EVENT_TYPE.ARTIST_CONCERT_OPEN){
                    await this.notificationService.sendArtistConcertOpenNotification(row.concertId);
                }else if(row.eventType === EVENT_TYPE.CONCERT_INFO_UPDATE){
                    await this.notificationService.sendConcertInfoUpdateNotification(row.concertId);
                }
            }catch(err){
                this.logger.warn(
        ...
        this.logger.log(`Processed ${rows.length} concert notification queue item(s)`);
    }
}
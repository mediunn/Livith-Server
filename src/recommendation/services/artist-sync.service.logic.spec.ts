...
describe('ArtistSyncService Logic Test ', () => {
    ...
    it('장르별로 200명씩 가져오는지 확인', async () => {
        ...
        mockPrisma.representativeArtist.upsert.mockImplementation(async (args: any) => {
    ...
    it('이미지 없는 아티스트 90개씩 처리', async () => {
        ...
        mockPrisma.representativeArtist.findMany.mockResolvedValue(mockArtists as any);
    ...
    it('장르명이 Last.fm 태그로 정확히 변환되는지 확인', async () => {
        ...
        jest.spyOn(service['lastfmApiService'], 'getTopArtistByTag')
